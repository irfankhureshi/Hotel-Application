...
const FeaturedDestination = () => {
  ...
  return (
    <div className='flex flex-col items-center px-6 md:px-16 lg:px-24 bg-slate-50'>
      ...
      <div className='grid grid-cols-2 items-center justify-center gap-8 mt-20 mb-10'>
        {roomsDummyData.slice(0,4).map((room, index)=>(
            <HotelCard key={room._id} room={room} index={index}/>
        ))}
      </div>

      <button onClick={()=>{navigate('/rooms'); scrollTo(0,0)}} className='my-16 px-4 py-2 text-sm font-medium border border-gray-400 rouded bg-white hover:bg-gray-100 transition-all cursor-pointer'>
